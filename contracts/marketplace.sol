...
contract MarketPlace is ReentrancyGuard {
    ...
    {
        try
            IERC721Receiver(purchaser).onERC721Received(
                purchaser,
                ownerERC721,
                assetID,
                ""
        ...
        returns (bytes4 response) {
            if (response != IERC721Receiver.onERC721Received.selector) {
                revert(
                    "_checkOnIERC721Receiver: ERC721Receiver rejected tokens"
                );
            }
    ...
    function _checkIERC1155Receiver(
        ...
        try
            IERC1155Receiver(purchaser).onERC1155Received(
                purchaser,
                ownerERC1155,
                assetID,
                quantity,
        ...
        returns (bytes4 response) {
            if (response != IERC1155Receiver.onERC1155Received.selector) {
                revert(
                    "_checkIERC1155Receiver: ERC1155Receiver rejected tokens"
                );
            }
        } catch Error(string memory reason) {
            revert(reason);
        } catch {
            revert("__checkIERC1155Receiver : contract not a ERC1155 reciver");
        }
    }
